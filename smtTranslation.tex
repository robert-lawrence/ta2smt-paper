...
\(exp\) are arithmetic formula over integer variables. As mentioned before,
...
increment between two adjacent time instances, or it must be reset. To maintain
a consistent view of time, we introduce
\(\delta: \mathbb{N} \rightarrow \mathbb{R}_{>0}\), which measures the amount of
time that elapses between two adjacent time instances. For a given clock
...
\begin{table}\label{tack-encoding}
  ...
  \begin{tabular}{c|c|c}
    ...
    \midrule
    \multicolumn{3}{c}{\(\varphi := \underset{q \in \mathcal{Q}_{k}}{\underset{k \in [1,K]}{\bigwedge}} \bigg( \Big( l[k] = q \land t[k] = \sharp \Big) \rightarrow \mathcal{X} \Big( Inv(q) \land r_{1}(Inv(q)) \Big) \bigg) \)} \\
    \midrule
    \multicolumn{3}{c}{\( \varphi := \underset{t \in T_{k}}{\underset{k \in [1,K]}{\bigwedge}} t[k] = t \rightarrow \Big( l[k] = t_{-} \land \mathcal{X}(l[k] = t_{+}) \land \varphi_{\gamma_{c}} \land \varphi_{\gamma_{v}} \land \varphi_{\alpha_{c}} \land \varphi_{\alpha_{v}}  \land \varphi_{edge}(t_{-}, t_{+}, k) \Big) \)} \\
  ...
  %  \multicolumn{3}{c}{\( \varphi_{\alpha](}(a,b,i) := Inv(a) \land r_{2}(Inv_{w}(b)) \land edge^{](}[i] \)} \\
    ...
    \midrule
    \multicolumn{3}{c}{\( \varphi := \underset{k \in [1,K]; q,q^{'} \in \mathcal{Q}_{k} | q \neq q^{'}}{\bigwedge} \bigg( \Big( (l[k] = q) \land \mathcal{X}(l[k] = q^{'}) \Big) \rightarrow \underset{t \in T_{k} | t_{-} = q, t_{+} = q^{'}}{\bigvee} (t[k] = t) \bigg) \)} \\
    \midrule
    \multicolumn{3}{c}{\( \varphi := \underset{x \in X}{\bigwedge} \bigg( \mathcal{X}(x_{0} = 0 \lor x_{1} = 0) \rightarrow \underset{t \in T_{k} | x \in t_{a_{c}}}{\underset{k \in [1,K]} {\bigvee}} t[k] = t \bigg) \)} \\
    \midrule
    \multicolumn{3}{c}{\( \varphi := \underset{n \in Int}{\bigwedge} \bigg( (\neg(n = \mathcal{X}(n))) \rightarrow \underset{t \in T_{k} | n \in t_{a_{v}}}{\underset{k \in [1,K]}{\bigvee}} t[k] = t \bigg) \)}
  \end{tabular}
\end{table}
...
\begin{table}
  ...
  \begin{tabular}{c c}
    ...
    \(\varphi_{1} \mathcal{U} \varphi_{2}\) & \(\overleftarrow{(\varphi_{1} \mathcal{U} \varphi_{2})}^{[k:0]} = \overleftarrow{(\varphi_{2})}^{[k:0]} \BitOr \bigl( \overleftarrow{(\varphi_{1})}^{[k:0]} \BitAnd \overleftarrow{(\varphi_{1} \mathcal{U} \varphi_{2})}^{[k{+}1:1]} \bigr) \) \\
             & \(\land \biggl( \Bigl( \big( \overleftarrow{(\varphi_{1})}^{[k{+}1]} \BitOr \overleftarrow{(\varphi_{2})}^{[k{+}1]} \BitOr !\overleftarrow{(\varphi_{1} \mathcal{U} \varphi_{2})}^{[k{+}1]} \big) \BitAnd\) \\
